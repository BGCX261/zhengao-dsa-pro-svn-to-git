...
#if 0	
	printf(" ---> 00  encode_fristfile %x \r\n",(u16)encode_fristfile);
	printf(" ---> 00  encode_filenum %x \r\n",(u16)encode_filenum);
...
//            break;
        ...
        case MSG_MUSIC_PLAY_NEW_FILE:									//按文件序号选择文件进行播放
            flush_all_msg();
            stop_decode();
	     if(DSA_SEL_DEV == (given_device & (~VIRTUAL_DEVICE))){
		//break;
	     }
...
//            disp_port(MENU_HALF_SEC_REFRESH);
	 ...
	 case MSG_10_PLUS:
	     next_prev_10_file(FIND_NEXT_10);		
	     break;
	 case MSG_10_MINUS:
	     next_prev_10_file(FIND_PREV_10);				
	     break;
#endif	
