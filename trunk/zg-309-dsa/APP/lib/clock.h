...
//#define SYSTEM_CLK_DIV4()  CLKCON1 |= 0x80; PCON |= BIT(1);timer_init(MAIN_CLK_12M)	 /*解码模式下不可使用*/
...
//#define WDT_CLEAR()     	WDTCON |= BIT(7)			//清WDT，必须在WDT溢出之前清掉WDT计数器
#define SYS_IDLE()         	PCON |= BIT(6)
#define SYSTEM_CLK_DIV4()  	CLKCON1 |= 0x80; PCON |= BIT(1);crc_key_prd(MAIN_CLK_12M);timer_init(MAIN_CLK_12M);UTBAUD = (12000000/(8 * 115200)) - 1	 /*解码模式下不可使用*/
...
#define PLL_IN_OSC1_12M      0x40			//crystal = 12M at osc1(选用P00、P01处使用12M晶振)
