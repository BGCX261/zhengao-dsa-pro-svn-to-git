...
xd_u8 id3_artist_len=0,id3_title_len=0,id3_alb_len=0;
...
{
    //printf_buf(decode_buffer + 512, 4);
    //printf_buf(decode_buffer + 516, 4);
}
void hunt_and_kill_wma_str()
{
	u8 i=1;
	...
	for(i=0;i<(id3_tag_len);i++){
		...
		if((id3_tag_buf[id3_tag_len-i]=='a')||(id3_tag_buf[id3_tag_len-i]=='A')){
			
			if((id3_tag_buf[id3_tag_len-i-1]=='m')||(id3_tag_buf[id3_tag_len-i-1]=='M')){
				
				if((id3_tag_buf[id3_tag_len-i-2]=='w')||(id3_tag_buf[id3_tag_len-i-2]=='W')){


					id3_tag_buf[id3_tag_len-i] = ' ';
					id3_tag_buf[id3_tag_len-i-1] = ' ';
					id3_tag_buf[id3_tag_len-i-2] = ' ';
						
					if(id3_tag_buf[id3_tag_len-i-3]=='.'){

						id3_tag_buf[id3_tag_len-i-3] = ' ';
						//i=i+4;
					}
...
void get_music_tag(void)
{
    ...
    {
        get_mp3_tag();
#ifdef ID3_UART_DEBUG		
	printf(" -get_mp3_tag--IS_MP3- \r\n");
#endif		
    }
...
#ifdef ID3_UART_DEBUG						
	printf(" ---show WMA name ----- \r\n");

	for(i=0;i<id3_tag_len;i++)
	printf("%c ",(char)(id3_tag_buf[i]));
	printf("\r\n");

	printf("%x ",(u16)id3_tag_len);
	printf("%x ",(u16)i);
#endif	
	hunt_and_kill_wma_str();	

#ifdef ID3_UART_DEBUG						
	printf(" ---hunt_and_kill_wma_str----- \r\n");
