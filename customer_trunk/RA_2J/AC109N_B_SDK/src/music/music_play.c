...
bool music_disp_file_dir=0;
xd_u8 ffr_func_flag=0;
...
void music_app_loop(void) AT(MUSIC_PLAY)
{
    ...
    while (1)
    {
	 ...
	 case MSG_MUSIC_STOP:
        ...
        case MSG_HALF_SECOND:
            //usb_timebase_detect();
		if(ffr_func_flag>0)ffr_func_flag--;            
	     pop_noise_handler();		            
            LED_FADE_OFF();                    
            UI_menu(MENU_MAIN);            
        ...
        case MSG_INPUT_TIMEOUT:
            ...
            break; 
#endif			
#ifdef DEVICE_ALTERNATE_SELECT_HOT_KEY
	...
	case MSG_NEXT_FOLDER:
		next_prev_folder(PLAY_NEXT_FILE);
		break;
	case MSG_PREV_FOLDER:
		next_prev_folder(PLAY_PREV_FILE);		
		break;
        default :
