...
extern xd_u16 band_top_freq,band_bottom_freq;	
...
bool chk_fm_dev_online(void) AT(FM_CODE)
{
    ...
    for (fm_mode_var.bAddr = 0; fm_mode_var.bAddr < (sizeof(fm_read_id)/2); fm_mode_var.bAddr++)
    {
        ...
        {
#ifdef FM_UART_DEBUG
	 	my_printf(" --->chk_fm_dev_online  addr  %x \r\n",(u16)fm_mode_var.bAddr);
#endif
            	return TRUE;
        }
    }	
...
bool set_fre(u8 mode) AT(FM_CODE)
{
    ...
    if (fm_mode_var.wFreq > band_top_freq)
        fm_mode_var.wFreq = band_bottom_freq;
    if (fm_mode_var.wFreq < band_bottom_freq)
        fm_mode_var.wFreq = band_top_freq;
...
{
    save_fm_point(fm_mode_var.wFreq - band_bottom_freq);
    fm_mode_var.bFreChannel = get_channel_via_fre(fm_mode_var.wFreq - band_bottom_freq);
...
void fm_info_init(void) AT(FM_CODE)
{
    ...
    if (fm_mode_var.wFreq > (band_top_freq - band_bottom_freq))
    {
        fm_mode_var.wFreq = band_bottom_freq;
    }
    else
...
bool fm_scan(u8 mode) AT(FM_CODE)
{
    ...
    {
        fm_module_mute(0);
        set_memory(MEM_FRE, fm_mode_var.wFreq - band_bottom_freq);
        save_fm_point(fm_mode_var.wFreq - band_bottom_freq);
        fm_mode_var.bFreChannel = get_channel_via_fre(fm_mode_var.wFreq - band_bottom_freq);
        fm_mode_var.bTotalChannel = get_total_mem_channel();
