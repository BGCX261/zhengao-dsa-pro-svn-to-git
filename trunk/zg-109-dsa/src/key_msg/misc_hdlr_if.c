...
void disp_bat_icon_buf(void)
{
	...
	LED5X7_clr_icon(LED_BAT_HALF);
	LED5X7_clr_icon(LED_BAT_FULL);
#ifdef BAT_LOW_LEVEL_BLINK_LED_ICON	
	LED5X7_clr_flash_icon(LED_BAT_HALF);
#endif

#ifdef USE_POWER_KEY
	if(work_mode==SYS_PWR_OFF_MODE)return;
#endif
	if(work_mode==SYS_IDLE_MODE)return;

	switch(batt_level)
	{
		case 0:
#ifdef BAT_LOW_LEVEL_BLINK_LED_ICON
			LED5X7_flash_icon(LED_BAT_HALF);
#else
			LED5X7_show_icon(LED_BAT_HALF);
#endif			
			break;
		case 1:
			LED5X7_show_icon(LED_BAT_HALF);
...
void bat_low_level_hldr(u8 bat_level)
{
	...
	if(bat_level<=BAT_LEVEL_ALERT){
   	...
   	my_printf(" --->bat_low_level_hldr :bat_low_level_cnt  %x \r\n",(u16)bat_low_level_cnt);
		...
		if(bat_low_level_cnt >=LOW_BAT_POWER_VALID_CNT){
