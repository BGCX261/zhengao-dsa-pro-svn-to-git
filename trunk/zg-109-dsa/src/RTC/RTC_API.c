...
{
	u8 tmp=0;
	tmp = read_RTC_reg(READ_SR0);
	tmp &=~(BIT(3)|BIT(4));
	tmp |=BIT(2);
	if(dir)
	{
		 write_RTC_reg(WRITE_SR0, tmp|BIT(1));
	}
	else
	{
		 write_RTC_reg(WRITE_SR0, (tmp&~(BIT(1)))|BIT(4));
	 }
}
#endif
...
void alarm_off(void) AT(RTC_CODE)
{
    ...
    else if (RTC_setting_var.bLastWorkMode != SYS_RTC_MODE)
    {
	 func_msg_hdlr(S_FUNC,(ENUM_WORK_MODE)RTC_setting_var.bLastWorkMode);
