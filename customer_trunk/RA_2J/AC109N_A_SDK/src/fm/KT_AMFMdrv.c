...
bool kt_band_mode=0;		
xd_u16 band_top_freq=0,band_bottom_freq=0;	
...
/*版    本：V1.0																	*/
/************************************************************************************/
u8 KT_AMFM_Init(void)                            //0->Fail 1->Success
{
	u16 regx;
	u16 i;
	for (i = 0;i < INIT_FAIL_TH;i++)
...
/*版    本：V1.0																	*/
/************************************************************************************/
void KT_AMFM_Standby(void)					//0->Fail 1->Success
{
	u16 regx;
...
/*版    本：V1.0																	*/
/************************************************************************************/
void KT_AMFM_WakeUp(void) //0->Fail 1->Success
{
	u16 regx;
...
*/
u8 KT_FM_Tune(u16 iFrequency) //87.5MHz-->Frequency=8750; Mute the chip and Tune to Frequency
{
	u16 reg60;
	char afc;
	char snr;
...
/*版    本：V2.2																	*/
/************************************************************************************/
u8 KT_AM_Tune(u16 iFrequency) //1710KHz --> Frequency=1710; Mute the chip and Tune to Frequency
{
//	char afc;
//	char snr;
	u16 reg1E;
...
/*版    本：V1.0																	*/
/************************************************************************************/
u8 KT_AM_ReadRSSI(char *RSSI) //range from -90 to -6, unit is dbm
{
	u16 regx;
...
/*版    本：V1.0																	*/
/************************************************************************************/
u8 KT_FM_ValidStation(u16 iFrequency) //0-->False Station 1-->Good Station //check AFC_DELTA only
{
#ifdef FMOLDSEEK
...
/*版    本：V1.0																	*/
/************************************************************************************/
u8 KT_AM_ValidStation(u16 iFrequency) //0-->False Station 1-->Good Station //check AFC_DELTA only
{
#ifdef AMOLDSEEK
...
/*版    本：V1.0																	*/
/************************************************************************************/
char KT_FM_GetAFC(void) // returned value's range is from -127 to 127, unit is KHz
{
	char afc_delta;
	u16 regx;
...
/*版    本：V1.0																	*/
/************************************************************************************/
char KT_AM_GetAFC(void) // returned value's range is from -127 to 127, unit is KHz
{
	char afc_delta;
	u16 regx;
...
void KT_band_mode_switch_profile(u8 band_mode)
{
	if(band_mode==FM_MODE){
		...
		band_top_freq = FM_BAND_TOP;
		band_bottom_freq = FM_BAND_BOTTOM;
		kt_band_mode = 0;
	...
	else if(band_mode==FM_MODE){

		band_top_freq = MW_BAND_TOP;
		band_bottom_freq = 	MW_BAND_BOTTOM;		
		kt_band_mode = 1;
