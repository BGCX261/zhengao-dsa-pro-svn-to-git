...
_near_func static void LCD_SEG5X9_show_dev(void) AT(LCD_SEG_CODE)
{
    ...
    if (device_active == DEVICE_SDMMCA)
     ...
     LCD_SEG5X9_var.state[3] |= COL_ICON;
}
...
void LCD_SEG5X9_show_music_main(void)
{
    ...
    if (Music_Play_var.bPlayStatus  == MAD_PLAY){
    	 LCD_SEG5X9_var.state[2] &= ~PAUS_ICON;
    	 LCD_SEG5X9_var.state[3] |= PLAY_ICON;
        //LCD_CON0 |= BIT(1);	            //[1]:DOTEN COM0-SEG0
    }
...
#if 1
    if(kt_band_mode){
	    LCD_SEG5X9_var.state[3] |=AM_ICON;
	    LCD_SEG5X9_var.state[0] |=KHZ_ICON;
    }
    else{		
	    LCD_SEG5X9_var.state[0] |= FM_ICON;
    }
#endif	
}
...
_near_func void LCD_drive5X9_12(void) AT(LCD_SEG_CODE)
{
    ...
    LCDC5X9_P1buf.bCom0Seg1 |= (LCD_SEG5X9_var.state[3] |LCD_SEG5X9_var.state[0]);
	
    LCDC5X9_P0buf.bCom0Seg0 = 0xFF;//(LCD_SEG5X9_var.state[0]);
    LCDC5X9_P0buf.bCom1Seg0 = 0xFF;//(LCD_SEG5X9_var.state[1]);
    LCDC5X9_P0buf.bCom2Seg0 = (LCD_SEG5X9_var.state[2]);
    LCDC5X9_P0buf.bCom3Seg0 = (LCD_SEG5X9_var.state[3]);
