...
static void music_info_init(void)
{
...
#endif		
    {
	  ...
	  if((get_device_online_status()&DEVICE_UDISK)>0){
        	given_device = DEVICE_UDISK;
    	  }	
    	 else if((get_device_online_status()&DEVICE_SDMMC0)>0){
...
////////////////////////////////////////////////////////////
//显示界面的切换
            if (main_menu_conter < (SUB_MENU_TIME+2))
            {
                main_menu_conter++;
            }
...
void music_decode(void)
{
    ...
    PT2313_Config(DECODE_CHAN,CHAN_ADJ);
