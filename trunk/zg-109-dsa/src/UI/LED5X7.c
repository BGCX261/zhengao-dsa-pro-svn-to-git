...
{
    0x77,0x7F,0x39,0x3f,0x79,///<ABCDE
...
const u8 LED_SMALL_LETTER[26] AT(LED_5X7_TABLE_CODE)=
{
    ...
    0x73,0x67,0x50,0x40,0x44,///<pqrst
...
void LED5X7_clr_icon(u8 icon) AT(LED_5X7_CODE)
{
    LED5X7_var.bShowBuff[4] &= ~(icon);
}
void LED5X7_show_icon(u8 icon) AT(LED_5X7_CODE)
{
    LED5X7_var.bShowBuff[4] |= (icon);
}
void LED5X7_clr_flash_icon(u8 icon) AT(LED_5X7_CODE)
{
    LED5X7_var.bFlashIcon &= ~(icon);
}
void LED5X7_flash_icon(u8 icon) AT(LED_5X7_CODE)
{
    LED5X7_var.bFlashIcon |= (icon);
}
/*----------------------------------------------------------------------------*/
...
#ifdef AUX_DISP_PAUSE_STR
	if(mute){		
	    	LED5X7_setX(0);
		LED5X7_clear_icon();			
		LED5X7_show_string((u8 *)"PAUS");
	}
#endif		
      	  LED_STATUS |= LED_AUX;
    }
...
extern void disp_bat_icon_buf(void);
#endif
/*----------------------------------------------------------------------------*/
/**@brief   LED扫描函数
   @param   void
...
_near_func void LED5X7_scan(void)  AT(COMMON_CODE)//AT(LED_5X7_CODE)
{
   ...
   disp_bat_icon_buf();
    ...
    led_gpio_custom_scan((BIT(cnt)) , ~seg);
