...
u8 _code DSA_Key[][3]={
  ...
  {0x55,0x1C,(MSG_REC_KEY)},
  {0x55,0x1D,(MSG_REC_PP)},
  {0x55,0x1E,(MSG_REC_STOP)},
  {0x55,0x1F,(MSG_DEL_CURR_FILE)},
  {0x55,0x20,(MSG_REC_MIC)},
  {0x55,0x21,(MSG_REC_AUX1)},
  {0x55,0x22,(MSG_REC_AUX2)},
			...
			switch(UI2DSA_STATE)	
			{
				...
				case 0xFC:
					DSA_flag|=0xFC<<8;
        				...
        				else if(RECODE_INIT== encode_status){
						DSA_flag |=0x08;
					}
        				else if(RECODE_PLAY== encode_status){
						DSA_flag |=0x10;
					}
				...
				case 0x30:
					...
					DSA_flag |= (fs_msg.dir_index)&0x0FFF;

					if(DSA_DATA[8]!= DSA_flag)
					DSA_Sdata=	 DSA_flag;    		
					DSA_DATA[8] = DSA_flag;
					UI2DSA_STATE=0x40;
					break;	
