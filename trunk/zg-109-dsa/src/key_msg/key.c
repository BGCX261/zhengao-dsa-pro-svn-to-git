...
{
    ADC_CH_FOR_KEY,               ///<高4位为延时，低4位为通道选择
...
_near_func void music_msg_filter(u8 key_status, u8 back_last_key) AT(KEY_CODE)
{
    ...
    if ((work_mode == SYS_USB_MODE)||(work_mode == SYS_SD_MODE))            //与其它模式共用，但只有music模式需要此处理
...
_near_func void key_scan(void) AT(COMMON_CODE)
{
    ...
    cur_key = AD_key(AD_var.wADValue[AD_CH_KEY]);       //4AD按键优先

    if (cur_key == NO_KEY)
    {
        cur_key = IR_key();                             //4检查是否有遥控
        if (cur_key != NO_KEY)
        {
    ...
    cur_key = key_filter(cur_key);                    //4过滤按键，相当于去抖
    
    back_last_key = last_key; 
    
    if (cur_key == last_key)                            //4长时间按键
    {   
        if (cur_key == NO_KEY)
            return;
        ...
        if (key_press_counter == KEY_LONG_CNT)          //4长按
        {
            key_status = KEY_LONG;
        }
        else if (key_press_counter == (KEY_LONG_CNT + KEY_HOLD_CNT))        //4连按
        {
            key_status = KEY_HOLD;
    ...
    else  //4cur_key = NO_KEY, 抬键
    {    
        last_key = cur_key; 
        
        if ((key_press_counter < KEY_LONG_CNT) && (cur_key == NO_KEY))      //4短按抬起
        {
            key_press_counter = 0;
